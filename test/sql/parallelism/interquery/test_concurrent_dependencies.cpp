...
static void create_drop_table(GuinsooDB *db) {
	Connection con(*db);
...
static void create_use_prepared_statement(GuinsooDB *db) {
	Connection con(*db);
...
static void create_drop_schema(GuinsooDB *db) {
	Connection con(*db);
...
static void create_use_table_view(GuinsooDB *db, int threadnr) {
	Connection con(*db);
	unique_ptr<QueryResult> result;
