...
static void read_from_integers(GuinsooDB *db, bool *correct, idx_t threadnr) {
	Connection con(*db);
...
static void append_to_integers(GuinsooDB *db, idx_t threadnr) {
	Connection con(*db);
...
static void append_to_primary_key(GuinsooDB *db) {
	Connection con(*db);
...
static void update_to_primary_key(GuinsooDB *db) {
	Connection con(*db);
	for (int32_t i = 0; i < 1000; i++) {
...
static void mix_insert_to_primary_key(GuinsooDB *db, atomic<idx_t> *count, idx_t thread_nr) {
	unique_ptr<QueryResult> result;
	Connection con(*db);
...
static void mix_update_to_primary_key(GuinsooDB *db, atomic<idx_t> *count, idx_t thread_nr) {
	unique_ptr<QueryResult> result;
	Connection con(*db);
...
static void append_to_primary_key_with_transaction(GuinsooDB *db, idx_t thread_nr, bool success[]) {
	Connection con(*db);
