...
public:
	...
	static void CheckpointThread(GuinsooDB *db, bool *read_correct) {
		Connection con(*db);
		while (!finished) {
			{
	...
	static void WriteRandomNumbers(GuinsooDB *db, bool *correct, size_t nr) {
		correct[nr] = true;
		Connection con(*db);
	...
	static void NopUpdate(GuinsooDB *db) {
		Connection con(*db);
