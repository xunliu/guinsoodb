...
static int guinsoodbConvertResult(MaterializedQueryResult &result, bool original_sqlite_test,
...
static Connection *GetConnection(GuinsooDB &db, unordered_map<string, unique_ptr<Connection>> &named_connection_map,
                                 string con_name) {
...
void Query::Execute() {
	...
	guinsoodbConvertResult(*result, runner.original_sqlite_test, azResult);
...
void SQLLogicTestRunner::LoadDatabase(string dbpath) {
	...
	db = make_unique<GuinsooDB>(dbpath, config.get());
...
void SQLLogicTestRunner::ExecuteFile(string script) {
	...
	const char *zDbEngine = "GuinsooDB";
