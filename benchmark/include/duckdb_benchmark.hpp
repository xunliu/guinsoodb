//===----------------------------------------------------------------------===//
//
//                         GuinsooDB
//
// guinsoodb_benchmark.hpp
//
// Author: Mark Raasveldt
//
//===----------------------------------------------------------------------===//

#pragma once

#include "benchmark.hpp"
#include "guinsoodb.hpp"
#include "guinsoodb/main/client_context.hpp"
#include "test_helpers.hpp"

namespace guinsoodb {

//! Base class for any state that has to be kept by a Benchmark
struct GuinsooDBBenchmarkState : public BenchmarkState {
	GuinsooDB db;
	Connection conn;
	unique_ptr<QueryResult> result;

	GuinsooDBBenchmarkState(string path) : db(path.empty() ? nullptr : path.c_str()), conn(db) {
		conn.EnableProfiling();
	}
	virtual ~GuinsooDBBenchmarkState() {
	}
};

//! The base Benchmark class is a base class that is used to create and register
//! new benchmarks
class GuinsooDBBenchmark : public Benchmark {
public:
	GuinsooDBBenchmark(bool register_benchmark, string name, string group) : Benchmark(register_benchmark, name, group) {
	}
	virtual ~GuinsooDBBenchmark() {
	}

	//! Load data into GuinsooDB
	virtual void Load(GuinsooDBBenchmarkState *state) = 0;
	//! Run a bunch of queries, only called if GetQuery() returns an empty string
	virtual void RunBenchmark(GuinsooDBBenchmarkState *state) {
	}
	//! This function gets called after the GetQuery() method
	virtual void Cleanup(GuinsooDBBenchmarkState *state) {};
	//! Verify a result
	virtual string VerifyResult(QueryResult *result) = 0;
	//! Whether or not the benchmark is performed on an in-memory database
	virtual bool InMemory() {
		return true;
	}

	string GetDatabasePath() {
		if (!InMemory()) {
			string path = "guinsoodb_benchmark_db.db";
			DeleteDatabase(path);
			return path;
		} else {
			return string();
		}
	}

	virtual unique_ptr<GuinsooDBBenchmarkState> CreateBenchmarkState() {
		return make_unique<GuinsooDBBenchmarkState>(GetDatabasePath());
	}

	unique_ptr<BenchmarkState> Initialize(BenchmarkConfiguration &config) override {
		auto state = CreateBenchmarkState();
		Load(state.get());
		return move(state);
	}

	void Run(BenchmarkState *state_p) override {
		auto state = (GuinsooDBBenchmarkState *)state_p;
		string query = GetQuery();
		if (query.empty()) {
			RunBenchmark(state);
		} else {
			state->result = state->conn.Query(query);
		}
	}

	void Cleanup(BenchmarkState *state_p) override {
		auto state = (GuinsooDBBenchmarkState *)state_p;
		Cleanup(state);
	}

	string Verify(BenchmarkState *state_p) override {
		auto state = (GuinsooDBBenchmarkState *)state_p;
		return VerifyResult(state->result.get());
	}

	string GetLogOutput(BenchmarkState *state_p) override {
		auto state = (GuinsooDBBenchmarkState *)state_p;
		return state->conn.context->profiler.ToJSON();
	}

	//! Interrupt the benchmark because of a timeout
	void Interrupt(BenchmarkState *state_p) override {
		auto state = (GuinsooDBBenchmarkState *)state_p;
		state->conn.Interrupt();
	}
};

} // namespace guinsoodb
