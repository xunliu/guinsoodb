...
#include "guinsoodb/storage/meta_block_writer.hpp"
#include "guinsoodb/storage/meta_block_reader.hpp"
#include "guinsoodb/common/exception.hpp"

#include "guinsoodb/common/serializer/buffered_deserializer.hpp"
...
MainHeader MainHeader::Deserialize(Deserializer &source) {
	...
	if (memcmp(magic_bytes, MainHeader::MAGIC_BYTES, MainHeader::MAGIC_BYTE_SIZE) != 0) {
		throw IOException("The file is not a valid GuinsooDB database file!");
	}
	...
	if (create_new) {
		...
		if (header.version_number != VERSION_NUMBER) {
			throw IOException(
			    ...
			    "The database file was created with an %s version of GuinsooDB.\n\n"
			    "The storage of GuinsooDB is not yet stable; newer versions of GuinsooDB cannot read old database files and "
			    "vice versa.\n"
			    "The storage will be stabilized when version 1.0 releases.\n\n"
			    "For now, we recommend that you load the database file in a supported version of GuinsooDB, and use the "
			    "EXPORT DATABASE command "
			    "followed by IMPORT DATABASE on the current version of GuinsooDB.",
