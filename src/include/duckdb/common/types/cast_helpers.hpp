...
//                         GuinsooDB
//
// guinsoodb/common/types/cast_helpers.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	...
	static char *FormatUnsigned(T value, char *ptr) {
		while (value >= 100) {
			...
			value /= 100;
			*--ptr = guinsoodb_fmt::internal::data::digits[index + 1];
			*--ptr = guinsoodb_fmt::internal::data::digits[index];
		}
		if (value < 10) {
	...
	static void Format(char *data, int32_t date[], idx_t year_length, bool add_bc) {
		...
		for (int i = 1; i <= 2; i++) {
			...
			if (date[i] < 10) {
				...
				ptr[1] = guinsoodb_fmt::internal::data::digits[index];
				ptr[2] = guinsoodb_fmt::internal::data::digits[index + 1];
			}
			ptr += 3;
		}
...
struct TimeToStringCast {
	...
	static void FormatTwoDigits(char *ptr, int32_t value) {
		...
		if (value < 10) {
			...
			auto index = static_cast<unsigned>(value * 2);
			ptr[0] = guinsoodb_fmt::internal::data::digits[index];
			ptr[1] = guinsoodb_fmt::internal::data::digits[index + 1];
		}
	}
