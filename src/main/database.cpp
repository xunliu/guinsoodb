...
#include "guinsoodb/parallel/task_scheduler.hpp"
...
#include "guinsoodb/main/connection_manager.hpp"
...
GuinsooDB::GuinsooDB(const char *path, DBConfig *new_config) : instance(make_shared<DatabaseInstance>()) {
	instance->Initialize(path, new_config);
}

GuinsooDB::GuinsooDB(const string &path, DBConfig *config) : GuinsooDB(path.c_str(), config) {
}
...
FileSystem &GuinsooDB::GetFileSystem() {
...
idx_t GuinsooDB::NumberOfThreads() {
