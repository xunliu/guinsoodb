...
struct GuinsooDBArrowSchemaHolder {
	// unused in children
...
static void ReleaseGuinsooDBArrowSchema(ArrowSchema *schema) {
	if (!schema || !schema->release) {
		return;
	}
	schema->release = nullptr;
	auto holder = static_cast<GuinsooDBArrowSchemaHolder *>(schema->private_data);
	delete holder;
}
...
void QueryResult::ToArrowSchema(ArrowSchema *out_schema) {
	...
	auto root_holder = make_unique<GuinsooDBArrowSchemaHolder>();
	...
	out_schema->name = "guinsoodb_query_result";
	...
	for (idx_t col_idx = 0; col_idx < ColumnCount(); col_idx++) {
		...
		child.release = ReleaseGuinsooDBArrowSchema;
	...
	out_schema->release = ReleaseGuinsooDBArrowSchema;
}
