...
void Transformer::TransformWindowDef(guinsoodb_libpgquery::PGWindowDef *window_spec, WindowExpression *expr) {
...
void Transformer::TransformWindowFrame(guinsoodb_libpgquery::PGWindowDef *window_spec, WindowExpression *expr) {
	D_ASSERT(window_spec);
	D_ASSERT(expr);
...
unique_ptr<ParsedExpression> Transformer::TransformFuncCall(guinsoodb_libpgquery::PGFuncCall *root) {
	auto name = root->funcname;
	string schema, function_name;
	if (name->length == 2) {
		// schema + name
		schema = reinterpret_cast<guinsoodb_libpgquery::PGValue *>(name->head->data.ptr_value)->val.str;
		function_name = reinterpret_cast<guinsoodb_libpgquery::PGValue *>(name->head->next->data.ptr_value)->val.str;
	} else {
		// unqualified name
		//		schema = DEFAULT_SCHEMA;
		schema = INVALID_SCHEMA;
		function_name = reinterpret_cast<guinsoodb_libpgquery::PGValue *>(name->head->data.ptr_value)->val.str;
	}
	...
	if (root->over) {
		...
		auto window_spec = reinterpret_cast<guinsoodb_libpgquery::PGWindowDef *>(root->over);
		if (window_spec->name) {
	...
	if (root->args != nullptr) {
		for (auto node = root->args->head; node != nullptr; node = node->next) {
			auto child_expr = TransformExpression((guinsoodb_libpgquery::PGNode *)node->data.ptr_value);
...
static string SQLValueOpToString(guinsoodb_libpgquery::PGSQLValueFunctionOp op) {
	switch (op) {
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_DATE:
		return "current_date";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_TIME:
		return "current_time";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_TIME_N:
		return "current_time_n";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_TIMESTAMP:
		return "current_timestamp";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_TIMESTAMP_N:
		return "current_timestamp_n";
	case guinsoodb_libpgquery::PG_SVFOP_LOCALTIME:
		return "current_localtime";
	case guinsoodb_libpgquery::PG_SVFOP_LOCALTIME_N:
		return "current_localtime_n";
	case guinsoodb_libpgquery::PG_SVFOP_LOCALTIMESTAMP:
		return "current_localtimestamp";
	case guinsoodb_libpgquery::PG_SVFOP_LOCALTIMESTAMP_N:
		return "current_localtimestamp_n";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_ROLE:
		return "current_role";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_USER:
		return "current_user";
	case guinsoodb_libpgquery::PG_SVFOP_USER:
		return "user";
	case guinsoodb_libpgquery::PG_SVFOP_SESSION_USER:
		return "session_user";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_CATALOG:
		return "current_catalog";
	case guinsoodb_libpgquery::PG_SVFOP_CURRENT_SCHEMA:
...
unique_ptr<ParsedExpression> Transformer::TransformSQLValueFunction(guinsoodb_libpgquery::PGSQLValueFunction *node) {
	if (!node) {
		return nullptr;
	}
	vector<unique_ptr<ParsedExpression>> children;
