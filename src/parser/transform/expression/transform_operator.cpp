...
unique_ptr<ParsedExpression> Transformer::TransformAExpr(guinsoodb_libpgquery::PGAExpr *root) {
	if (!root) {
		return nullptr;
	}
	auto name = string((reinterpret_cast<guinsoodb_libpgquery::PGValue *>(root->name->head->data.ptr_value))->val.str);

	switch (root->kind) {
	case guinsoodb_libpgquery::PG_AEXPR_IN: {
		...
		TransformExpressionList((guinsoodb_libpgquery::PGList *)root->rexpr, result->children);
		return move(result);
	}
	// rewrite NULLIF(a, b) into CASE WHEN a=b THEN NULL ELSE a END
	case guinsoodb_libpgquery::PG_AEXPR_NULLIF: {
		vector<unique_ptr<ParsedExpression>> children;
	...
	case guinsoodb_libpgquery::PG_AEXPR_BETWEEN:
	case guinsoodb_libpgquery::PG_AEXPR_NOT_BETWEEN: {
		auto between_args = reinterpret_cast<guinsoodb_libpgquery::PGList *>(root->rexpr);
		...
		auto between_left =
		    TransformExpression(reinterpret_cast<guinsoodb_libpgquery::PGNode *>(between_args->head->data.ptr_value));
		auto between_right =
		    TransformExpression(reinterpret_cast<guinsoodb_libpgquery::PGNode *>(between_args->tail->data.ptr_value));
		...
		auto compare_between = make_unique<ConjunctionExpression>(ExpressionType::CONJUNCTION_AND, move(compare_left),
		                                                          move(compare_right));
		if (root->kind == guinsoodb_libpgquery::PG_AEXPR_BETWEEN) {
	...
	case guinsoodb_libpgquery::PG_AEXPR_SIMILAR: {
		auto left_expr = TransformExpression(root->lexpr);
