...
unique_ptr<CreateStatement> Transformer::TransformCreateFunction(guinsoodb_libpgquery::PGNode *node) {
	D_ASSERT(node);
	D_ASSERT(node->type == guinsoodb_libpgquery::T_PGCreateFunctionStmt);

	auto stmt = reinterpret_cast<guinsoodb_libpgquery::PGCreateFunctionStmt *>(node);
	D_ASSERT(stmt);
