...
void Transformer::TransformCopyOptions(CopyInfo &info, guinsoodb_libpgquery::PGList *options) {
	if (!options) {
		return;
	}
	...
	for_each_cell(cell, options->head) {
		...
		if (StringUtil::Lower(def_elem->defname) == "format") {
			...
			auto *format_val = (guinsoodb_libpgquery::PGValue *)(def_elem->arg);
			if (!format_val || format_val->type != guinsoodb_libpgquery::T_PGString) {
		...
		case guinsoodb_libpgquery::T_PGList: {
			auto column_list = (guinsoodb_libpgquery::PGList *)(def_elem->arg);
		...
		default:
			info.options[def_elem->defname].push_back(
			    TransformValue(*((guinsoodb_libpgquery::PGValue *)def_elem->arg))->value);
			break;
		}
	}
}

unique_ptr<CopyStatement> Transformer::TransformCopy(guinsoodb_libpgquery::PGNode *node) {
	auto stmt = reinterpret_cast<guinsoodb_libpgquery::PGCopyStmt *>(node);
	D_ASSERT(stmt);
	...
	if (stmt->attlist) {
		for (auto n = stmt->attlist->head; n != nullptr; n = n->next) {
			auto target = reinterpret_cast<guinsoodb_libpgquery::PGResTarget *>(n->data.ptr_value);
			if (target->name) {
	...
	if (stmt->relation) {
		...
		result->select_statement = TransformSelectNode((guinsoodb_libpgquery::PGSelectStmt *)stmt->query);
	}
