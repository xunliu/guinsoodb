...
unique_ptr<CreateStatement> Transformer::TransformCreateIndex(guinsoodb_libpgquery::PGNode *node) {
	auto stmt = reinterpret_cast<guinsoodb_libpgquery::PGIndexStmt *>(node);
	D_ASSERT(stmt);
	...
	for (auto cell = stmt->indexParams->head; cell != nullptr; cell = cell->next) {
		auto index_element = (guinsoodb_libpgquery::PGIndexElem *)cell->data.ptr_value;
