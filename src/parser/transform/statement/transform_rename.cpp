...
unique_ptr<AlterStatement> Transformer::TransformRename(guinsoodb_libpgquery::PGNode *node) {
	auto stmt = reinterpret_cast<guinsoodb_libpgquery::PGRenameStmt *>(node);
	D_ASSERT(stmt);
	...
	case guinsoodb_libpgquery::PG_OBJECT_COLUMN: {
		// change column name
	...
	case guinsoodb_libpgquery::PG_OBJECT_DATABASE:
	default:
