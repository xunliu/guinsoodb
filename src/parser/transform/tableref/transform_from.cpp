...
unique_ptr<TableRef> Transformer::TransformFrom(guinsoodb_libpgquery::PGList *root) {
	if (!root) {
	...
	auto n = reinterpret_cast<guinsoodb_libpgquery::PGNode *>(root->head->data.ptr_value);
