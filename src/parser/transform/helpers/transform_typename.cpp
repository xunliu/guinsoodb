...
LogicalType Transformer::TransformTypeName(guinsoodb_libpgquery::PGTypeName *type_name) {
	auto name = (reinterpret_cast<guinsoodb_libpgquery::PGValue *>(type_name->names->tail->data.ptr_value)->val.str);
	...
	if (base_type == LogicalTypeId::STRUCT) {
		...
		for (auto node = type_name->typmods->head; node; node = node->next) {
			auto &type_val = *((guinsoodb_libpgquery::PGList *)node->data.ptr_value);
			D_ASSERT(type_val.length == 2);

			auto entry_name_node = (guinsoodb_libpgquery::PGValue *)(type_val.head->data.ptr_value);
			D_ASSERT(entry_name_node->type == guinsoodb_libpgquery::T_PGString);
			auto entry_type_node = (guinsoodb_libpgquery::PGValue *)(type_val.tail->data.ptr_value);
			D_ASSERT(entry_type_node->type == guinsoodb_libpgquery::T_PGTypeName);
			...
			auto entry_type = TransformTypeName((guinsoodb_libpgquery::PGTypeName *)entry_type_node);
	...
	if (base_type == LogicalTypeId::MAP) {
		...
		auto key_type = TransformTypeName((guinsoodb_libpgquery::PGTypeName *)type_name->typmods->head->data.ptr_value);
		auto value_type = TransformTypeName((guinsoodb_libpgquery::PGTypeName *)type_name->typmods->tail->data.ptr_value);
	...
	if (type_name->typmods) {
		for (auto node = type_name->typmods->head; node; node = node->next) {
			auto &const_val = *((guinsoodb_libpgquery::PGAConst *)node->data.ptr_value);
			if (const_val.type != guinsoodb_libpgquery::T_PGAConst ||
			    const_val.val.type != guinsoodb_libpgquery::T_PGInteger) {
