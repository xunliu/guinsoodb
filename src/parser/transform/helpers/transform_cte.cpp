...
#include "guinsoodb/parser/query_node/recursive_cte_node.hpp"

namespace guinsoodb {

void Transformer::TransformCTE(guinsoodb_libpgquery::PGWithClause *de_with_clause, QueryNode &select) {
	...
	for (auto cte_ele = de_with_clause->ctes->head; cte_ele != nullptr; cte_ele = cte_ele->next) {
		...
		auto cte = reinterpret_cast<guinsoodb_libpgquery::PGCommonTableExpr *>(cte_ele->data.ptr_value);
		...
		// we need a query
		if (!cte->ctequery || cte->ctequery->type != guinsoodb_libpgquery::T_PGSelectStmt) {
...
unique_ptr<SelectStatement> Transformer::TransformRecursiveCTE(guinsoodb_libpgquery::PGCommonTableExpr *cte,
                                                               CommonTableExpressionInfo &info) {
	auto stmt = (guinsoodb_libpgquery::PGSelectStmt *)cte->ctequery;
