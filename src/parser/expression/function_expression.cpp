...
hash_t FunctionExpression::Hash() const {
	...
	result = CombineHash(result, guinsoodb::Hash<const char *>(schema.c_str()));
	result = CombineHash(result, guinsoodb::Hash<const char *>(function_name.c_str()));
	result = CombineHash(result, guinsoodb::Hash<bool>(distinct));
	return result;
}
