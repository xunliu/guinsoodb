...
struct FMTPrintf {
	template <class CTX>
	static string OP(const char *format_str, std::vector<guinsoodb_fmt::basic_format_arg<CTX>> &format_args) {
		return guinsoodb_fmt::vsprintf(
		    format_str, guinsoodb_fmt::basic_format_args<CTX>(format_args.data(), static_cast<int>(format_args.size())));
	}
};
...
static void PrintfFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	for (idx_t idx = 0; idx < count; idx++) {
		...
		std::vector<guinsoodb_fmt::basic_format_arg<CTX>> format_args;
		...
		for (idx_t col_idx = 1; col_idx < args.ColumnCount(); col_idx++) {
			...
			case LogicalTypeId::BOOLEAN: {
				...
				format_args.emplace_back(guinsoodb_fmt::internal::make_arg<CTX>(arg_data[arg_idx]));
				break;
			}
			case LogicalTypeId::TINYINT: {
			...
			case LogicalTypeId::VARCHAR: {
				...
				auto string_view =
				    guinsoodb_fmt::basic_string_view<char>(arg_data[arg_idx].GetDataUnsafe(), arg_data[arg_idx].GetSize());
				format_args.emplace_back(guinsoodb_fmt::internal::make_arg<CTX>(string_view));
				break;
			}
			default:
...
void PrintfFun::RegisterFunction(BuiltinFunctions &set) {
	// guinsoodb_fmt::printf_context, guinsoodb_fmt::vsprintf
	ScalarFunction printf_fun =
	    ScalarFunction("printf", {LogicalType::VARCHAR}, LogicalType::VARCHAR,
	                   PrintfFunction<FMTPrintf, guinsoodb_fmt::printf_context>, false, BindPrintfFunction);
	printf_fun.varargs = LogicalType::ANY;
	set.AddFunction(printf_fun);

	// guinsoodb_fmt::format_context, guinsoodb_fmt::vformat
	ScalarFunction format_fun =
	    ScalarFunction("format", {LogicalType::VARCHAR}, LogicalType::VARCHAR,
	                   PrintfFunction<FMTFormat, guinsoodb_fmt::format_context>, false, BindPrintfFunction);
