#include "guinsoodb/common/gzip_stream.hpp"

#include "guinsoodb/common/exception.hpp"
#include "guinsoodb/common/file_system.hpp"
#include "guinsoodb/common/fstream_util.hpp"
...
void GzipStreamBuf::Initialize() {
	...
	mz_stream_ptr = new guinsoodb_miniz::mz_stream();
	...
	auto ret = guinsoodb_miniz::mz_inflateInit2((guinsoodb_miniz::mz_streamp)mz_stream_ptr, -MZ_DEFAULT_WINDOW_BITS);
	if (ret != guinsoodb_miniz::MZ_OK) {
...
std::streambuf::int_type GzipStreamBuf::underflow() {
	...
	auto zstrm_p = (guinsoodb_miniz::mz_streamp)mz_stream_ptr;
	if (!zstrm_p) {
	...
	if (gptr() == egptr()) {
		...
		do {
			...
			auto ret = guinsoodb_miniz::mz_inflate(zstrm_p, guinsoodb_miniz::MZ_NO_FLUSH);
			if (ret != guinsoodb_miniz::MZ_OK && ret != guinsoodb_miniz::MZ_STREAM_END) {
				throw Exception(guinsoodb_miniz::mz_error(ret));
			}
			...
			if (ret == guinsoodb_miniz::MZ_STREAM_END) {
				guinsoodb_miniz::mz_inflateEnd(zstrm_p);
				delete zstrm_p;
