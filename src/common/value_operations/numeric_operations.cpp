...
#include "guinsoodb/common/operator/numeric_binary_operators.hpp"
#include "guinsoodb/common/value_operations/value_operations.hpp"
#include "guinsoodb/common/operator/aggregate_operators.hpp"
#include "guinsoodb/common/operator/add.hpp"
#include "guinsoodb/common/operator/multiply.hpp"
#include "guinsoodb/common/operator/subtract.hpp"
...
Value ValueOperations::Add(const Value &left, const Value &right) {
	return BinaryValueOperation<guinsoodb::AddOperator>(left, right);
}

Value ValueOperations::Subtract(const Value &left, const Value &right) {
	return BinaryValueOperation<guinsoodb::SubtractOperator>(left, right);
}

Value ValueOperations::Multiply(const Value &left, const Value &right) {
	return BinaryValueOperation<guinsoodb::MultiplyOperator>(left, right);
}
...
Value ValueOperations::Modulo(const Value &left, const Value &right) {
	if (right == 0) {
		...
		return BinaryValueOperation<guinsoodb::ModuloOperator>(left, right);
	}
}
...
Value ValueOperations::Divide(const Value &left, const Value &right) {
	if (right == 0) {
		...
		return BinaryValueOperation<guinsoodb::DivideOperator>(left, right);
	}
}
