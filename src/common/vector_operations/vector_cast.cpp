...
#include "guinsoodb/common/types/cast_helpers.hpp"
#include "guinsoodb/common/types/chunk_collection.hpp"
#include "guinsoodb/common/types/decimal.hpp"
...
static void NumericCastSwitch(Vector &source, Vector &result, idx_t count) {
	...
	switch (result.GetType().id()) {
	case LogicalTypeId::BOOLEAN:
		UnaryExecutor::Execute<SRC, bool, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::TINYINT:
		UnaryExecutor::Execute<SRC, int8_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::SMALLINT:
		UnaryExecutor::Execute<SRC, int16_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::INTEGER:
		UnaryExecutor::Execute<SRC, int32_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::BIGINT:
		UnaryExecutor::Execute<SRC, int64_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::UTINYINT:
		UnaryExecutor::Execute<SRC, uint8_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::USMALLINT:
		UnaryExecutor::Execute<SRC, uint16_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::UINTEGER:
		UnaryExecutor::Execute<SRC, uint32_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::UBIGINT:
		UnaryExecutor::Execute<SRC, uint64_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::HUGEINT:
		UnaryExecutor::Execute<SRC, hugeint_t, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::FLOAT:
		UnaryExecutor::Execute<SRC, float, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::DOUBLE:
		UnaryExecutor::Execute<SRC, double, guinsoodb::Cast>(source, result, count);
		break;
	case LogicalTypeId::DECIMAL:
		ToDecimalCast<SRC>(source, result, count);
		break;
	case LogicalTypeId::VARCHAR: {
		VectorStringCast<SRC, guinsoodb::StringCast>(source, result, count);
		break;
	}
...
static void StringCastSwitch(Vector &source, Vector &result, idx_t count, bool strict = false) {
	...
	case LogicalTypeId::DATE:
		if (strict) {
			UnaryExecutor::Execute<string_t, date_t, guinsoodb::StrictCastToDate>(source, result, count);
		} else {
			UnaryExecutor::Execute<string_t, date_t, guinsoodb::CastToDate>(source, result, count);
		}
		break;
	case LogicalTypeId::TIME:
		if (strict) {
			UnaryExecutor::Execute<string_t, dtime_t, guinsoodb::StrictCastToTime>(source, result, count);
		} else {
			UnaryExecutor::Execute<string_t, dtime_t, guinsoodb::CastToTime>(source, result, count);
		}
		break;
	case LogicalTypeId::TIMESTAMP:
		UnaryExecutor::Execute<string_t, timestamp_t, guinsoodb::CastToTimestamp>(source, result, count);
		break;
	case LogicalTypeId::BLOB:
		VectorStringCast<string_t, guinsoodb::CastToBlob>(source, result, count);
		break;
	default:
		if (strict) {
			VectorStringCastNumericSwitch<guinsoodb::StrictCast>(source, result, count);
		} else {
			VectorStringCastNumericSwitch<guinsoodb::Cast>(source, result, count);
		}
		break;
	}
}
...
static void DateCastSwitch(Vector &source, Vector &result, idx_t count) {
	...
	case LogicalTypeId::VARCHAR:
		// date to varchar
		VectorStringCast<date_t, guinsoodb::CastFromDate>(source, result, count);
		break;
	case LogicalTypeId::TIMESTAMP:
		// date to timestamp
		UnaryExecutor::Execute<date_t, timestamp_t, guinsoodb::CastDateToTimestamp>(source, result, count);
		break;
	default:
		VectorNullCast(source, result, count);
		break;
...
static void TimeCastSwitch(Vector &source, Vector &result, idx_t count) {
	...
	case LogicalTypeId::VARCHAR:
		// time to varchar
		VectorStringCast<dtime_t, guinsoodb::CastFromTime>(source, result, count);
		break;
	default:
		VectorNullCast(source, result, count);
		break;
...
static void TimestampCastSwitch(Vector &source, Vector &result, idx_t count) {
	...
	case LogicalTypeId::VARCHAR:
		...
		VectorStringCast<timestamp_t, guinsoodb::CastFromTimestamp>(source, result, count);
		break;
	case LogicalTypeId::DATE:
		// timestamp to date
		UnaryExecutor::Execute<timestamp_t, date_t, guinsoodb::CastTimestampToDate>(source, result, count);
		break;
	case LogicalTypeId::TIME:
		// timestamp to time
		UnaryExecutor::Execute<timestamp_t, dtime_t, guinsoodb::CastTimestampToTime>(source, result, count);
		break;
	default:
		VectorNullCast(source, result, count);
		break;
...
static void IntervalCastSwitch(Vector &source, Vector &result, idx_t count) {
	...
	case LogicalTypeId::VARCHAR:
		// time to varchar
		VectorStringCast<interval_t, guinsoodb::StringCast>(source, result, count);
		break;
	default:
		VectorNullCast(source, result, count);
		break;
...
static void BlobCastSwitch(Vector &source, Vector &result, idx_t count) {
	...
	case LogicalTypeId::VARCHAR:
		// blob to varchar
		VectorStringCast<string_t, guinsoodb::CastFromBlob>(source, result, count);
		break;
	default:
		VectorNullCast(source, result, count);
		break;
