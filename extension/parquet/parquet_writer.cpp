...
using namespace guinsoodb_miniz;              // NOLINT
...
static Type::type GuinsooDBTypeToParquetType(const LogicalType &guinsoodb_type) {
	switch (guinsoodb_type.id()) {
	...
	case LogicalTypeId::TIMESTAMP:
		return Type::INT96;
	default:
		throw NotImplementedException(guinsoodb_type.ToString());
	}
}

static bool GuinsooDBTypeToConvertedType(const LogicalType &guinsoodb_type, ConvertedType::type &result) {
	switch (guinsoodb_type.id()) {
	case LogicalTypeId::VARCHAR:
...
ParquetWriter::ParquetWriter(FileSystem &fs, string file_name_p, vector<LogicalType> types_p, vector<string> names_p,
	...
	for (idx_t i = 0; i < sql_types.size(); i++) {
		...
		schema_element.type = GuinsooDBTypeToParquetType(sql_types[i]);
		...
		schema_element.__isset.converted_type = GuinsooDBTypeToConvertedType(sql_types[i], schema_element.converted_type);
	}
}
...
void ParquetWriter::Flush(ChunkCollection &buffer) {
	...
	for (idx_t i = 0; i < buffer.ColumnCount(); i++) {
		...
		case CompressionCodec::ZSTD: {
			compressed_size = guinsoodb_zstd::ZSTD_compressBound(temp_writer.blob.size);
			compressed_buf = unique_ptr<data_t[]>(new data_t[compressed_size]);
			compressed_size = guinsoodb_zstd::ZSTD_compress((void *)compressed_buf.get(), compressed_size,
