# this script creates a single header + source file combination out of the GuinsooDB sources
import os
import re
import sys
import shutil
...
header_file = os.path.join(amal_dir, "guinsoodb.hpp")
source_file = os.path.join(amal_dir, "guinsoodb.cpp")
temp_header = 'guinsoodb.hpp.tmp'
temp_source = 'guinsoodb.cpp.tmp'

skip_guinsoodb_includes = False
...
# files included in the amalgamated "guinsoodb.hpp" file
main_header_files = [os.path.join(include_dir, 'guinsoodb.hpp'),
    os.path.join(include_dir, 'guinsoodb.h'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'types', 'date.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'arrow.hpp'),
	os.path.join(include_dir, 'guinsoodb', 'common', 'types', 'decimal.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'types', 'hugeint.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'types', 'interval.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'types', 'timestamp.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'types', 'time.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'serializer', 'buffered_file_writer.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'common', 'serializer', 'buffered_serializer.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'main', 'appender.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'main', 'client_context.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'function', 'function.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'function', 'table_function.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'parser', 'parsed_data', 'create_table_function_info.hpp'),
    os.path.join(include_dir, 'guinsoodb', 'parser', 'parsed_data', 'create_copy_function_info.hpp')]
...
if '--extended' in sys.argv:
    ...
    main_header_files += [os.path.join(include_dir, x) for x in [
        ...
        'guinsoodb/planner/expression/bound_function_expression.hpp',
        ...
        'guinsoodb/storage/data_table.hpp',
        ...
        "guinsoodb/storage/statistics/string_statistics.hpp",
        "guinsoodb/storage/statistics/numeric_statistics.hpp"]]
    main_header_files += add_include_dir(os.path.join(include_dir, 'guinsoodb/parser/expression'))
    main_header_files += add_include_dir(os.path.join(include_dir, 'guinsoodb/parser/parsed_data'))
    main_header_files += add_include_dir(os.path.join(include_dir, 'guinsoodb/parser/tableref'))
...
always_excluded = ['src/amalgamation/guinsoodb.cpp', 'src/amalgamation/guinsoodb.hpp', 'src/amalgamation/parquet-amalgamation.cpp', 'src/amalgamation/parquet-amalgamation.hpp']
...
excluded_compilation_files = excluded_files + ['gram.hpp', 'kwlist.hpp', "guinsoodb-c.cpp"]
...
def get_includes(fpath, text):
    ...
    for x in regex_include_statements:
        included_file = x[1]
        if skip_guinsoodb_includes and 'guinsoodb' in included_file:
            continue
...
def generate_guinsoodb_hpp(header_file):
    print("-----------------------")
...
def generate_amalgamation(source_file, header_file):
    # construct guinsoodb.hpp from these headers
    generate_guinsoodb_hpp(header_file)

    # now construct guinsoodb.cpp
    print("------------------------")
