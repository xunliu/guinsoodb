...
command_prefix = ['java']
if persistent:
	command_prefix += ['-Dguinsoodb.database.file=/tmp/lancer_guinsoodb_db']
command_prefix += ['-jar', os.path.join(targetdir, found_filename)]

command = '--num-threads 1 --random-seed 0 --log-each-select=true --print-statements=true --timeout-seconds 600 guinsoodb'.split(' ')
