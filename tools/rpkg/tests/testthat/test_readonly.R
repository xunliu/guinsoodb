test_that("read_only flag and shutdown works as expected", {
  ...
  callr::r(function(dbdir) {
    con <- DBI::dbConnect(guinsoodb::guinsoodb(), dbdir, read_only = FALSE) # FALSE is the default
    print(con)
    res <- DBI::dbWriteTable(con, "iris", iris)
    DBI::dbDisconnect(con)
    guinsoodb::guinsoodb_shutdown(con@driver)
  }, args = list(dbdir))


  # 2nd: start two parallel read-only references
  drv <- guinsoodb::guinsoodb(dbdir, read_only = TRUE)
  con <- dbConnect(drv)
  ...
  # con is still alive
  callr::r(function(dbdir) {
    con <- DBI::dbConnect(guinsoodb::guinsoodb(), dbdir, read_only = TRUE)
