test_that("guinsoodb_read_csv() works as expected", {
  ...
  guinsoodb_read_csv(con, "iris", tf)
  ...
  guinsoodb_read_csv(con, "iris", tf, delim = " ")
  res <- dbReadTable(con, "iris")
  res$Species <- as.factor(res$Species)
  expect_true(identical(res, iris))
  dbRemoveTable(con, "iris")
  ...
  write.table(iris, tf, row.names = FALSE, sep = " ")
  guinsoodb_read_csv(con, "iris", tf, sep = " ")
  res <- dbReadTable(con, "iris")
  res$Species <- as.factor(res$Species)
  expect_true(identical(res, iris))
  dbRemoveTable(con, "iris")
  ...
  guinsoodb_read_csv(con, "iris", tf, header = FALSE)
  res <- dbReadTable(con, "iris")
  ...
  # lowercase header
  write.csv(iris, tf, row.names = FALSE)
  guinsoodb_read_csv(con, "iris", tf, lower.case.names = T)
  res <- dbReadTable(con, "iris")
  res$species <- as.factor(res$species)
  iris_lc <- iris
  ...
  guinsoodb_read_csv(con, "iris", tf, na.strings = "NULL")
  res <- dbReadTable(con, "iris")
  res$Species <- as.factor(res$Species)
  expect_true(identical(res, iris_na))
  dbRemoveTable(con, "iris")
  ...
  # strange table name
  write.csv(iris, tf, row.names = FALSE)
  guinsoodb_read_csv(con, "ir Is", tf)
  ...
  write.csv(iris, tf, row.names = FALSE)
  guinsoodb_read_csv(con, "iris", tf, col.names = colnames)
  res <- dbReadTable(con, "iris")
  ...
  guinsoodb_read_csv(con, "iris", c(tf, tf2))
  count <- dbGetQuery(con, "SELECT COUNT(*) FROM iris")[1][1]
  expect_true(identical(as.integer(count), as.integer(nrow(iris) * 2)))
  dbRemoveTable(con, "iris")
  ...
  # deprecated API
  write.csv(iris, tf, row.names = FALSE)
  expect_warning(read_csv_guinsoodb(con, tf, "iris"), "guinsoodb_read_csv")
  res <- dbReadTable(con, "iris")
  res$Species <- as.factor(res$Species)
  expect_true(identical(res, iris))
