...
static R_altrep_class_t guinsoodb_altrep_string_class;

struct GuinsooDBAltrepStringWrapper {
	vector<Vector> vectors;
	idx_t length;
};

static GuinsooDBAltrepStringWrapper *guinsoodb_altrep_wrapper(SEXP x) {
	auto wrapper = (GuinsooDBAltrepStringWrapper *)R_ExternalPtrAddr(R_altrep_data1(x));
	if (!wrapper) {
...
static R_xlen_t guinsoodb_altrep_strings_length(SEXP x) {
	return guinsoodb_altrep_wrapper(x)->length;
}
...
static void *guinsoodb_altrep_strings_dataptr(SEXP x, Rboolean writeable) {
	auto *wrapper = guinsoodb_altrep_wrapper(x);
...
static const void *guinsoodb_altrep_strings_dataptr_or_null(SEXP x) {
	return nullptr;
}

static SEXP guinsoodb_altrep_strings_elt(SEXP x, R_xlen_t i) {
	auto *wrapper = guinsoodb_altrep_wrapper(x);
	if (R_altrep_data2(x) != R_NilValue) {
...
static void guinsoodb_altrep_strings_set_elt(SEXP x, R_xlen_t i, SEXP val) {
	guinsoodb_altrep_strings_dataptr(x, TRUE);
	SET_STRING_ELT(R_altrep_data2(x), i, val);
}

static int guinsoodb_altrep_strings_is_sorted(SEXP x) {
	// we don't know
	return 0;
}

static int guinsoodb_altrep_strings_no_na(SEXP x) {
	// we kinda know but it matters little
	return 0;
}

static void guinsoodb_altrep_strings_finalizer(SEXP x) {
	auto *wrapper = (GuinsooDBAltrepStringWrapper *)R_ExternalPtrAddr(x);
	if (wrapper) {
...
SEXP guinsoodb_release_R(SEXP stmtsexp) {
	if (TYPEOF(stmtsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_release_R: Need external pointer parameter");
	}
...
SEXP guinsoodb_finalize_statement_R(SEXP stmtsexp) {
	return guinsoodb_release_R(stmtsexp);
}

SEXP guinsoodb_prepare_R(SEXP connsexp, SEXP querysexp) {
	RProtector r;
	if (TYPEOF(querysexp) != STRSXP || Rf_length(querysexp) != 1) {
		Rf_error("guinsoodb_prepare_R: Need single string parameter for query");
	}
	if (TYPEOF(connsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_prepare_R: Need external pointer parameter for connections");
	}

	char *query = (char *)CHAR(STRING_ELT(querysexp, 0));
	if (!query) {
		Rf_error("guinsoodb_prepare_R: No query");
	}

	Connection *conn = (Connection *)R_ExternalPtrAddr(connsexp);
	if (!conn) {
		Rf_error("guinsoodb_prepare_R: Invalid connection");
	}

	auto stmt = conn->Prepare(query);
	if (!stmt->success) {
		Rf_error("guinsoodb_prepare_R: Failed to prepare query %s\nError: %s", query, stmt->error.c_str());
	}
	...
	R_RegisterCFinalizer(stmtsexp, (void (*)(SEXP))guinsoodb_finalize_statement_R);
	...
	for (auto &stype : stmtholder->stmt->GetTypes()) {
		...
		case LogicalTypeId::BLOB:
			...
			break;
		default:
			Rf_error("guinsoodb_prepare_R: Unknown column type for prepare: %s", stype.ToString().c_str());
			break;
		}
...
SEXP guinsoodb_bind_R(SEXP stmtsexp, SEXP paramsexp) {
	if (TYPEOF(stmtsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_bind_R: Need external pointer parameter");
	}
	RStatement *stmtholder = (RStatement *)R_ExternalPtrAddr(stmtsexp);
	if (!stmtholder || !stmtholder->stmt) {
		Rf_error("guinsoodb_bind_R: Invalid statement");
	}
	...
	if (stmtholder->stmt->n_param == 0) {
		Rf_error("guinsoodb_bind_R: dbBind called but query takes no parameters");
		return R_NilValue;
	}

	if (TYPEOF(paramsexp) != VECSXP || (idx_t)Rf_length(paramsexp) != stmtholder->stmt->n_param) {
		Rf_error("guinsoodb_bind_R: bind parameters need to be a list of length %i", stmtholder->stmt->n_param);
	}
	...
	for (idx_t param_idx = 0; param_idx < (idx_t)Rf_length(paramsexp); param_idx++) {
		...
		if (Rf_length(valsexp) != 1) {
			Rf_error("guinsoodb_bind_R: bind parameter values need to have length 1");
		}
		...
		default:
			Rf_error("guinsoodb_bind_R: Unsupported parameter type");
		}
...
SEXP guinsoodb_execute_R_impl(MaterializedQueryResult *result);

SEXP guinsoodb_execute_R(SEXP stmtsexp) {
	if (TYPEOF(stmtsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_execute_R: Need external pointer parameter");
	}
	RStatement *stmtholder = (RStatement *)R_ExternalPtrAddr(stmtsexp);
	if (!stmtholder || !stmtholder->stmt) {
		Rf_error("guinsoodb_execute_R: Invalid statement");
	}
...

	{
		...
		if (!generic_result->success) {
			Rf_error("guinsoodb_execute_R: Failed to run query\nError: %s", generic_result->error.c_str());
		}
		...
		out = r.Protect(guinsoodb_execute_R_impl(result));
	}
...
SEXP guinsoodb_execute_R_impl(MaterializedQueryResult *result) {
	...
	for (size_t col_idx = 0; col_idx < ncols; col_idx++) {
		...
		case LogicalTypeId::TIME:
			...
			break;
		case LogicalTypeId::VARCHAR: {
			auto wrapper = new GuinsooDBAltrepStringWrapper();
			...
			R_RegisterCFinalizer(ptr, guinsoodb_altrep_strings_finalizer);
			varvalue = r_varvalue.Protect(R_new_altrep(guinsoodb_altrep_string_class, ptr, R_NilValue));
			UNPROTECT(1);
			break;
		}
		...
		case LogicalTypeId::BLOB:
			...
			break;
		default:
			Rf_error("guinsoodb_execute_R: Unknown column type for execute: %s",
	...
	while (true) {
		...
		for (size_t col_idx = 0; col_idx < chunk->ColumnCount(); col_idx++) {
			...
			case LogicalTypeId::DOUBLE:
				...
				break;
			case LogicalTypeId::VARCHAR: {
				auto wrapper = (GuinsooDBAltrepStringWrapper *)R_ExternalPtrAddr(R_altrep_data1(dest));
			...
			default:
				Rf_error("guinsoodb_execute_R: Unknown column type for convert: %s",
...
static SEXP guinsoodb_finalize_database_R(SEXP dbsexp) {
	if (TYPEOF(dbsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_finalize_connection_R: Need external pointer parameter");
	}
	GuinsooDB *dbaddr = (GuinsooDB *)R_ExternalPtrAddr(dbsexp);
	if (dbaddr) {
		Rf_warning("guinsoodb_finalize_database_R: Database is garbage-collected, use dbDisconnect(con, shutdown=TRUE) or "
		           "guinsoodb::guinsoodb_shutdown(drv) to avoid this.");
...
struct DataFrameScanFunction : public TableFunction {
	...
	static unique_ptr<FunctionData> dataframe_scan_bind(ClientContext &context, vector<Value> &inputs,
		...
		for (idx_t col_idx = 0; col_idx < (idx_t)Rf_length(df); col_idx++) {
			...
			LogicalType guinsoodb_col_type;
			switch (rtypes[col_idx]) {
			case RType::LOGICAL:
				guinsoodb_col_type = LogicalType::BOOLEAN;
				break;
			case RType::INTEGER:
				guinsoodb_col_type = LogicalType::INTEGER;
				break;
			case RType::NUMERIC:
				guinsoodb_col_type = LogicalType::DOUBLE;
				break;
			case RType::FACTOR:
			case RType::STRING:
				guinsoodb_col_type = LogicalType::VARCHAR;
				break;
			case RType::TIMESTAMP:
				guinsoodb_col_type = LogicalType::TIMESTAMP;
				break;
			case RType::TIME_SECONDS:
			case RType::TIME_MINUTES:
			case RType::TIME_HOURS:
			case RType::TIME_DAYS:
			case RType::TIME_WEEKS:
				guinsoodb_col_type = LogicalType::TIME;
				break;
			case RType::DATE:
				guinsoodb_col_type = LogicalType::DATE;
				break;
			default:
				Rf_error("Unsupported column type for scan");
			}
			return_types.push_back(guinsoodb_col_type);
		}
...
SEXP guinsoodb_startup_R(SEXP dbdirsexp, SEXP readonlysexp) {
	if (TYPEOF(dbdirsexp) != STRSXP || Rf_length(dbdirsexp) != 1) {
		Rf_error("guinsoodb_startup_R: Need string parameter for dbdir");
	}
	char *dbdir = (char *)CHAR(STRING_ELT(dbdirsexp, 0));

	if (TYPEOF(readonlysexp) != LGLSXP || Rf_length(readonlysexp) != 1) {
		Rf_error("guinsoodb_startup_R: Need string parameter for read_only");
	}
	...
	GuinsooDB *dbaddr;
	try {
		dbaddr = new GuinsooDB(dbdir, &config);
	} catch (exception &e) {
		Rf_error("guinsoodb_startup_R: Failed to open database: %s", e.what());
	}
	...
	R_RegisterCFinalizer(dbsexp, (void (*)(SEXP))guinsoodb_finalize_database_R);
	return dbsexp;
}

SEXP guinsoodb_shutdown_R(SEXP dbsexp) {
	if (TYPEOF(dbsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_finalize_connection_R: Need external pointer parameter");
	}
	GuinsooDB *dbaddr = (GuinsooDB *)R_ExternalPtrAddr(dbsexp);
...
static SEXP guinsoodb_finalize_connection_R(SEXP connsexp) {
	if (TYPEOF(connsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_finalize_connection_R: Need external pointer parameter");
	}
	Connection *connaddr = (Connection *)R_ExternalPtrAddr(connsexp);
	if (connaddr) {
		Rf_warning("guinsoodb_finalize_connection_R: Connection is garbage-collected, use dbDisconnect() to avoid this.");
...
SEXP guinsoodb_register_R(SEXP connsexp, SEXP namesexp, SEXP valuesexp) {
	if (TYPEOF(connsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_register_R: Need external pointer parameter for connection");
	}
	Connection *conn = (Connection *)R_ExternalPtrAddr(connsexp);
	if (!conn) {
		Rf_error("guinsoodb_register_R: Invalid connection");
	}
	if (TYPEOF(namesexp) != STRSXP || Rf_length(namesexp) != 1) {
		Rf_error("guinsoodb_register_R: Need single string parameter for name");
	}
	auto name = string(CHAR(STRING_ELT(namesexp, 0)));
	if (name.empty()) {
		Rf_error("guinsoodb_register_R: name parameter cannot be empty");
	}
	if (TYPEOF(valuesexp) != VECSXP || Rf_length(valuesexp) < 1 ||
	    strcmp("data.frame", CHAR(STRING_ELT(GET_CLASS(valuesexp), 0))) != 0) {
		Rf_error("guinsoodb_register_R: Need at least one-column data frame parameter for value");
	}
	try {
		...
		Rf_error("guinsoodb_register_R: Failed to register data frame: %s", e.what());
	}
	return R_NilValue;
}

SEXP guinsoodb_unregister_R(SEXP connsexp, SEXP namesexp) {
	if (TYPEOF(connsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_unregister_R: Need external pointer parameter for connection");
	}
	Connection *conn = (Connection *)R_ExternalPtrAddr(connsexp);
	if (!conn) {
		Rf_error("guinsoodb_unregister_R: Invalid connection");
	}
	if (TYPEOF(namesexp) != STRSXP || Rf_length(namesexp) != 1) {
		Rf_error("guinsoodb_unregister_R: Need single string parameter for name");
	}
	auto name = string(CHAR(STRING_ELT(namesexp, 0)));
	auto key = Rf_install(("_registered_df_" + name).c_str());
...
SEXP guinsoodb_connect_R(SEXP dbsexp) {
	if (TYPEOF(dbsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_connect_R: Need external pointer parameter");
	}
	GuinsooDB *dbaddr = (GuinsooDB *)R_ExternalPtrAddr(dbsexp);
	if (!dbaddr) {
		Rf_error("guinsoodb_connect_R: Invalid database reference");
	}

	RProtector r;
	SEXP connsexp = r.Protect(R_MakeExternalPtr(new Connection(*dbaddr), R_NilValue, R_NilValue));
	R_RegisterCFinalizer(connsexp, (void (*)(SEXP))guinsoodb_finalize_connection_R);

	return connsexp;
}

SEXP guinsoodb_disconnect_R(SEXP connsexp) {
	if (TYPEOF(connsexp) != EXTPTRSXP) {
		Rf_error("guinsoodb_disconnect_R: Need external pointer parameter");
	}
	Connection *connaddr = (Connection *)R_ExternalPtrAddr(connsexp);
	if (connaddr) {
		R_ClearExternalPtr(connsexp);
...
SEXP guinsoodb_ptr_to_str(SEXP extptr) {
	if (TYPEOF(extptr) != EXTPTRSXP) {
		Rf_error("guinsoodb_ptr_to_str: Need external pointer parameter");
	}
	RProtector r;
...
#define CALLDEF(name, n)                                                                                               \
	{ #name, (DL_FUNC)&name, n }
static const R_CallMethodDef R_CallDef[] = {CALLDEF(guinsoodb_startup_R, 2),
                                            CALLDEF(guinsoodb_connect_R, 1),
                                            CALLDEF(guinsoodb_prepare_R, 2),
                                            CALLDEF(guinsoodb_bind_R, 2),
                                            CALLDEF(guinsoodb_execute_R, 1),
                                            CALLDEF(guinsoodb_release_R, 1),
                                            CALLDEF(guinsoodb_register_R, 3),
                                            CALLDEF(guinsoodb_unregister_R, 2),
                                            CALLDEF(guinsoodb_disconnect_R, 1),
                                            CALLDEF(guinsoodb_shutdown_R, 1),
                                            CALLDEF(guinsoodb_ptr_to_str, 1),

                                            {NULL, NULL, 0}};

void R_init_guinsoodb(DllInfo *dll) {
	R_registerRoutines(dll, NULL, R_CallDef, NULL, NULL);
	R_useDynamicSymbols(dll, FALSE);

	guinsoodb_altrep_string_class = R_make_altstring_class("guinsoodb_strings", "guinsoodb", dll);

	/* override ALTREP methods */
	R_set_altrep_Inspect_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_inspect);
	R_set_altrep_Length_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_length);

	/* override ALTVEC methods */
	R_set_altvec_Dataptr_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_dataptr);
	R_set_altvec_Dataptr_or_null_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_dataptr_or_null);

	/* override ALTSTRING methods */
	R_set_altstring_Elt_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_elt);
	R_set_altstring_Is_sorted_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_is_sorted);
	R_set_altstring_No_NA_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_no_na);
	R_set_altstring_Set_elt_method(guinsoodb_altrep_string_class, guinsoodb_altrep_strings_set_elt);
