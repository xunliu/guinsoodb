...
if 'GUINSOODB_R_BINDIR' in os.environ and 'GUINSOODB_R_CFLAGS' in os.environ and 'GUINSOODB_R_LIBS' in os.environ:
    ...
    # find libraries
    result_libs = package_build.get_libraries(existing_guinsoodb_dir, libraries, extensions)
...
guinsoodb_sources = [package_build.get_relative_path(os.path.join(script_path, 'src'), x) for x in source_list]
object_list = ' '.join([x.rsplit('.', 1)[0] + '.o' for x in guinsoodb_sources])

# include list
include_list = ' '.join(['-I' + 'guinsoodb/' + x for x in include_list])
include_list += ' -Iguinsoodb'
