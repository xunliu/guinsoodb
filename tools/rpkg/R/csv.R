#' Reads a CSV file into GuinsooDB
#'
#' Directly reads a CSV file into GuinsooDB, tries to detect and create the correct schema for it.
#' This usually is much faster than reading the data into R and writing it to GuinsooDB.
#'
#' @inheritParams guinsoodb_register
...
#'
#' guinsoodb_read_csv(con, "data", path)
#' dbReadTable(con, "data")
#'
#' dbDisconnect(con)
guinsoodb_read_csv <- function(conn, name, files, header = TRUE, na.strings = "", nrow.check = 500,
...
#'
#' `read_csv_guinsoodb()` has been superseded by `guinsoodb_read_csv()`.
#' The order of the arguments has changed.
#'
#' @rdname deprecated
#' @export
#' @keywords internal
read_csv_guinsoodb <- function(conn, files, tablename, ...) {
  .Deprecated(
    "guinsoodb_read_csv", old = "read_csv_guinsoodb",
    msg = "Use 'guinsoodb_read_csv' instead, with changed order of arguments."
  )
  # Different order of arguments!
  guinsoodb_read_csv(conn, tablename, files, ...)
}
