#' GuinsooDB driver class
#'
#' Implements \linkS4class{DBIDriver}.
#'
#' @aliases guinsoodb_driver
#' @keywords internal
#' @export
setClass("guinsoodb_driver", contains = "DBIDriver", slots = list(database_ref = "externalptr", dbdir = "character", read_only = "logical"))

#' GuinsooDB connection class
#'
#' Implements \linkS4class{DBIConnection}.
#'
#' @aliases guinsoodb_connection
#' @keywords internal
#' @export
setClass("guinsoodb_connection",
  contains = "DBIConnection",
  slots = list(conn_ref = "externalptr",
               driver = "guinsoodb_driver",
               debug = "logical",
               timezone_out = "character",
               tz_out_convert = "character")
)

guinsoodb_connection <- function(guinsoodb_driver, debug) {
  new(
    "guinsoodb_connection",
    conn_ref = .Call(guinsoodb_connect_R, guinsoodb_driver@database_ref),
    driver = guinsoodb_driver,
    debug = debug,
    timezone_out = "UTC",
    tz_out_convert = "with"
...
#' @rdname guinsoodb_connection-class
#' @inheritParams methods::show
#' @export
setMethod(
  "show", "guinsoodb_connection",
  function(object) {
    message(sprintf("<guinsoodb_connection %s driver=%s>", extptr_str(object@conn_ref), drv_to_string(object@driver)))
    invisible(NULL)
  }
)
...
#' @export
setMethod(
  "dbIsValid", "guinsoodb_connection",
  function(dbObj, ...) {
    valid <- FALSE
    tryCatch(
      {
...
#' @export
setMethod(
  "dbSendQuery", c("guinsoodb_connection", "character"),
  function(conn, statement, params = NULL, ...) {
    if (conn@debug) {
      message("Q ", statement)
    }
    statement <- enc2utf8(statement)
    stmt_lst <- .Call(guinsoodb_prepare_R, conn@conn_ref, statement)

    res <- guinsoodb_result(
      connection = conn,
      stmt_lst = stmt_lst
    )
    if (length(params) > 0) {
...
#' @export
setMethod(
  "dbDataType", "guinsoodb_connection",
  function(dbObj, obj, ...) {
    dbDataType(dbObj@driver, obj, ...)
  }
)

guinsoodb_random_string <- function(x) {
...
#' @export
setMethod(
  "dbWriteTable", c("guinsoodb_connection", "character", "data.frame"),
  function(conn,
           name,
           value,
           row.names = FALSE,
    ...
    view_name <- sprintf("_guinsoodb_append_view_%s", guinsoodb_random_string())
    on.exit(guinsoodb_unregister(conn, view_name))
    guinsoodb_register(conn, view_name, value)
...
#' @export
setMethod(
  "dbListTables", "guinsoodb_connection",
  function(conn, ...) {
    dbGetQuery(
      conn,
      SQL(
...
#' @export
setMethod(
  "dbExistsTable", c("guinsoodb_connection", "character"),
  function(conn, name, ...) {
    if (!dbIsValid(conn)) {
...
#' @export
setMethod(
  "dbListFields", c("guinsoodb_connection", "character"),
  function(conn, name, ...) {
    names(dbGetQuery(
      conn,
      sqlInterpolate(
...
#' @export
setMethod(
  "dbRemoveTable", c("guinsoodb_connection", "character"),
  function(conn, name, ...) {
    dbExecute(
      conn,
...
#' @export
setMethod(
  "dbGetInfo", "guinsoodb_connection",
  function(dbObj, ...) {
...
#' @export
setMethod(
  "dbRollback", "guinsoodb_connection",
  function(conn, ...) {
