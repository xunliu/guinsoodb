...
def guinsoodb_empty_cursor(request):
    test_dbfarm = tmp_path.resolve().as_posix()

    connection = guinsoodb.connect('')
    cursor = connection.cursor()
    return cursor

@pytest.fixture(scope="function")
def guinsoodb_cursor(request):
...
@pytest.fixture(scope="function")
def guinsoodb_cursor_autocommit(request, tmp_path):
    test_dbfarm = tmp_path.resolve().as_posix()
    ...
    connection = guinsoodb.connect(test_dbfarm)
...
@pytest.fixture(scope="function")
def initialize_guinsoodb(request, tmp_path):
    test_dbfarm = tmp_path.resolve().as_posix()
