...
class TestArrow(object):
    def test_arrow(self, guinsoodb_cursor):
        if not can_run:
            return

        parquet_filename = 'userdata1.parquet'
        urllib.request.urlretrieve('https://github.com/cwida/guinsoodb-data/releases/download/v1.0/userdata1.parquet', parquet_filename)
        ...
        rel_from_arrow = guinsoodb.arrow(userdata_parquet_table).project(cols).arrow()
        rel_from_arrow.validate(full=True)

        rel_from_guinsoodb = guinsoodb.from_parquet(parquet_filename).project(cols).arrow()
        rel_from_guinsoodb.validate(full=True)
        ...
        for i in [7, 51, 99, 100, 101, 500, 1000, 2000]:
            ...
            rel_from_arrow2 = guinsoodb.arrow(userdata_parquet_table2).project(cols).arrow()
            rel_from_arrow2.validate(full=True)

            assert rel_from_arrow.equals(rel_from_arrow2, check_metadata=True)
            assert rel_from_arrow.equals(rel_from_guinsoodb, check_metadata=True)

    def test_arrow_unsigned(self,guinsoodb_cursor):
        if not can_run:
            return
        ...
        rel_from_arrow = guinsoodb.arrow(unsigned_parquet_table).project(cols).arrow()
        rel_from_arrow.validate(full=True)
        ...
        round_tripping = guinsoodb.from_arrow_table(arrow_result).to_arrow_table()
