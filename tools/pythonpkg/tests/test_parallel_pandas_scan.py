...
def run_parallel_queries(main_table, left_join_table, expected_df, iteration_count = 5):
    for i in range(0, iteration_count):
        ...
        """
        try:
            guinsoodb_conn = guinsoodb.connect()
            guinsoodb_conn.execute("PRAGMA threads=4")
            guinsoodb_conn.register('main_table', main_table)
            guinsoodb_conn.register('left_join_table', left_join_table)
            output_df = guinsoodb_conn.execute(sql).fetchdf()
            pd.testing.assert_frame_equal(expected_df, output_df)
            print(output_df)
        except Exception as err:
            print(err)
        finally:
            guinsoodb_conn.close()

class TestParallelPandasScan(object):
    def test_parallel_numeric_scan(self, guinsoodb_cursor):
    ...
    def test_parallel_complex_unicode_text(self, guinsoodb_cursor):
    ...
    def test_parallel_emojis(self, guinsoodb_cursor):
        if sys.version_info.major < 3:
            return
    ...
    def test_parallel_numeric_object(self, guinsoodb_cursor):
    ...
    def test_parallel_timestamp(self, guinsoodb_cursor):
