...
static bool ConvertColumnRegular(idx_t target_offset, data_ptr_t target_data, bool *target_mask, VectorData &idata,
                                 idx_t count) {
	return ConvertColumn<T, T, guinsoodb_py_convert::RegularConvert>(target_offset, target_data, target_mask, idata,
	                                                              count);
}
...
RawArrayWrapper::RawArrayWrapper(const LogicalType &type) : data(nullptr), type(type), count(0) {
	...
	case LogicalTypeId::BLOB:
		...
		break;
	default:
		throw std::runtime_error("Unsupported type " + type.ToString() + " for GuinsooDB -> NumPy conversion");
	}
}
...
void ArrayWrapper::Append(idx_t current_offset, Vector &input, idx_t count) {
	...
	case LogicalTypeId::UBIGINT:
		...
		break;
	case LogicalTypeId::HUGEINT:
		may_have_null = ConvertColumn<hugeint_t, double, guinsoodb_py_convert::IntegralConvert>(current_offset, dataptr,
		                                                                                     maskptr, idata, count);
		break;
	case LogicalTypeId::FLOAT:
	...
	case LogicalTypeId::DECIMAL:
		...
		break;
	case LogicalTypeId::TIMESTAMP:
		may_have_null = ConvertColumn<timestamp_t, int64_t, guinsoodb_py_convert::TimestampConvert>(
		    current_offset, dataptr, maskptr, idata, count);
		break;
	case LogicalTypeId::DATE:
		may_have_null = ConvertColumn<date_t, int64_t, guinsoodb_py_convert::DateConvert>(current_offset, dataptr, maskptr,
		                                                                               idata, count);
		break;
	case LogicalTypeId::TIME:
		may_have_null = ConvertColumn<dtime_t, PyObject *, guinsoodb_py_convert::TimeConvert>(current_offset, dataptr,
		                                                                                   maskptr, idata, count);
		break;
	case LogicalTypeId::VARCHAR:
		may_have_null = ConvertColumn<string_t, PyObject *, guinsoodb_py_convert::StringConvert>(current_offset, dataptr,
		                                                                                      maskptr, idata, count);
		break;
	case LogicalTypeId::BLOB:
		may_have_null = ConvertColumn<string_t, PyObject *, guinsoodb_py_convert::BlobConvert>(current_offset, dataptr,
		                                                                                    maskptr, idata, count);
		break;
	default:
