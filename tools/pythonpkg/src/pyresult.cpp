...
void GuinsooDBPyResult::Initialize(py::handle &m) {
	py::class_<GuinsooDBPyResult>(m, "GuinsooDBPyResult")
	    .def("close", &GuinsooDBPyResult::Close)
	    .def("fetchone", &GuinsooDBPyResult::Fetchone)
	    .def("fetchall", &GuinsooDBPyResult::Fetchall)
	    .def("fetchnumpy", &GuinsooDBPyResult::FetchNumpy)
	    .def("fetchdf", &GuinsooDBPyResult::FetchDF)
	    .def("fetch_df", &GuinsooDBPyResult::FetchDF)
	    .def("fetch_df_chunk", &GuinsooDBPyResult::FetchDFChunk)
	    .def("fetch_arrow_table", &GuinsooDBPyResult::FetchArrowTable)
	    .def("arrow", &GuinsooDBPyResult::FetchArrowTable)
	    .def("df", &GuinsooDBPyResult::FetchDF);
...
py::object GuinsooDBPyResult::Fetchone() {
	if (!result) {
	...
	for (idx_t col_idx = 0; col_idx < result->types.size(); col_idx++) {
		...
		case LogicalTypeId::TIME: {
			...
			guinsoodb::Time::Convert(time, hour, min, sec, microsec);
		...
		case LogicalTypeId::DATE: {
			...
			guinsoodb::Date::Convert(date, year, month, day);
...
py::list GuinsooDBPyResult::Fetchall() {
	py::list res;
	while (true) {
		auto fres = Fetchone();
		if (fres.is_none()) {
...
py::dict GuinsooDBPyResult::FetchNumpy(bool stream) {
	if (!result) {
		throw std::runtime_error("result closed");
	}
...
py::list GuinsooDBPyResult::Description() {
