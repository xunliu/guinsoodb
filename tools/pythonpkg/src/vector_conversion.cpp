#include "guinsoodb_python/vector_conversion.hpp"
...
void VectorConversion::NumpyToGuinsooDB(PandasColumnBindData &bind_data, py::array &numpy_col, idx_t count, idx_t offset,
                                     Vector &out) {
...
static void ConvertPandasType(const string &col_type, LogicalType &guinsoodb_col_type, PandasType &pandas_type) {
	if (col_type == "bool") {
		...
		guinsoodb_col_type = LogicalType::BIGINT;
		pandas_type = PandasType::BIGINT;
	} else if (col_type == "float32") {
		guinsoodb_col_type = LogicalType::FLOAT;
...
void VectorConversion::BindPandas(py::handle df, vector<PandasColumnBindData> &bind_columns,
	...
	for (idx_t col_idx = 0; col_idx < py::len(df_columns); col_idx++) {
		...
		if (col_type == "Int8" || col_type == "Int16" || col_type == "Int32" || col_type == "Int64") {
			...
			ConvertPandasType(col_type, guinsoodb_col_type, bind_data.pandas_type);
			...
			if (col_type == "category") {
				...
				ConvertPandasType(category_type, guinsoodb_col_type, bind_data.pandas_type);
			} else {
				ConvertPandasType(col_type, guinsoodb_col_type, bind_data.pandas_type);
			}
		}
