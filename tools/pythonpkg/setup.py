...
for i in range(len(sys.argv)):
    ...
    if sys.argv[i].startswith("--binary-dir="):
        existing_guinsoodb_dir = sys.argv[i].split('=', 1)[1]
        recognized = True
...
main_source_files = ['guinsoodb_python.cpp'] + [os.path.join('src', x) for x in os.listdir(main_source_path) if '.cpp' in x]
include_directories = [main_include_path, get_numpy_include(), get_pybind_include(), get_pybind_include(user=True)]
if len(existing_guinsoodb_dir) == 0:
    ...
    if os.path.isfile(os.path.join(script_path, '..', '..', 'scripts', 'amalgamation.py')):
        ...
        (source_list, include_list, original_sources) = package_build.build_package(os.path.join(script_path, 'guinsoodb'), extensions)

        guinsoodb_sources = [os.path.sep.join(package_build.get_relative_path(script_path, x).split('/')) for x in source_list]
        guinsoodb_sources.sort()

        original_sources = [os.path.join('guinsoodb', x) for x in original_sources]

        guinsoodb_includes = [os.path.join('guinsoodb', x) for x in include_list]
        guinsoodb_includes += ['guinsoodb']

        # gather the include files
        import amalgamation
        header_files = amalgamation.list_includes_files(guinsoodb_includes)

        # write the source list, include list and git hash to separate files
        with open_utf8('sources.list', 'w+') as f:
            for source_file in guinsoodb_sources:
                f.write(source_file + "\n")

        with open_utf8('includes.list', 'w+') as f:
            for include_file in guinsoodb_includes:
        ...
        with open_utf8('sources.list', 'r') as f:
            guinsoodb_sources = [x for x in f.read().split('\n') if len(x) > 0]

        with open_utf8('includes.list', 'r') as f:
            guinsoodb_includes = [x for x in f.read().split('\n') if len(x) > 0]

    source_files += guinsoodb_sources
    include_directories = guinsoodb_includes + include_directories

    libguinsoodb = Extension('guinsoodb',
    ...
    result_libraries = package_build.get_libraries(existing_guinsoodb_dir, libraries, extensions)
...
setup(
    name = "guinsoodb",
    description = 'GuinsooDB embedded database',
    keywords = 'GuinsooDB Database SQL OLAP',
    url="https://www.guinsoodb.org",
    long_description = 'See here for an introduction: https://guinsoodb.org/docs/api/python',
    license='MIT',
    install_requires=[ # these version is still available for Python 2, newer ones aren't
         'numpy>=1.14'
    ],
    data_files = data_files,
    packages=['guinsoodb_query_graph'],
