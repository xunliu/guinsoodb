...
# Files are served at https://download.guinsoodb.org/...
...
def curlcmd(cmd, path):
    p = subprocess.Popen(['curl','--retry', '10'] + cmd + ['http://guinsoodb:%s@dav10635776.mywebdav.de/guinsoodb-download/%s' % (secret_key, path)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
for f in files:
    ...
    key = '%s/%s' % (folder, base)
    print("%s\t->\thttps://download.guinsoodb.org/%s " % (f, key))
    curlcmd(['-T',f], key)
