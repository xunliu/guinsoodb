...
struct ConnectTask : public Task {
	...
	void DoWork() override {
		...
		connection.connection = guinsoodb::make_unique<guinsoodb::Connection>(*connection.database_ref->database);
	}
};
...
Connection::Connection(const Napi::CallbackInfo &info) : Napi::ObjectWrap<Connection>(info) {
	...
	database_ref->Schedule(env, guinsoodb::make_unique<ConnectTask>(*this, callback));
}
...
struct ExecTask : public Task {
	...
	void DoWork() override {
		...
		// thanks Mark
		for (guinsoodb::idx_t i = 0; i < statements.size(); i++) {
...
Napi::Value Connection::Exec(const Napi::CallbackInfo &info) {
	...
	database_ref->Schedule(info.Env(), guinsoodb::make_unique<ExecTask>(*this, sql, callback));
	return Value();
}
