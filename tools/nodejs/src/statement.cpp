...
Statement::Statement(const Napi::CallbackInfo &info) : Napi::ObjectWrap<Statement>(info) {
	...
	connection_ref->database_ref->Schedule(env, guinsoodb::make_unique<PrepareTask>(*this, callback));
}
...
static guinsoodb::Value bind_parameter(const Napi::Value source) {
	if (source.IsString()) {
		return guinsoodb::Value(source.As<Napi::String>().Utf8Value());
	} else if (other_instance_of(source.As<Napi::Object>(), "RegExp")) {
		return guinsoodb::Value(source.ToString().Utf8Value());
	} else if (source.IsNumber()) {
		if (Utils::OtherIsInt(source.As<Napi::Number>())) {
			return guinsoodb::Value::INTEGER(source.As<Napi::Number>().Int32Value());
		} else {
			return guinsoodb::Value::DOUBLE(source.As<Napi::Number>().DoubleValue());
		}
	} else if (source.IsBoolean()) {
		return guinsoodb::Value::BOOLEAN(source.As<Napi::Boolean>().Value());
	} else if (source.IsNull()) {
		return guinsoodb::Value();
	} else if (source.IsBuffer()) {
		Napi::Buffer<char> buffer = source.As<Napi::Buffer<char>>();
		return guinsoodb::Value::BLOB(std::string(buffer.Data(), buffer.Length()));
...
static Napi::Value convert_chunk(Napi::Env &env, std::vector<std::string> names, guinsoodb::DataChunk &chunk) {
	...
	for (guinsoodb::idx_t row_idx = 0; row_idx < chunk.size(); row_idx++) {
		Napi::Object row_result = Napi::Object::New(env);

		for (guinsoodb::idx_t col_idx = 0; col_idx < chunk.ColumnCount(); col_idx++) {
			Napi::Value value;
...
struct StatementParam {
	std::vector<guinsoodb::Value> params;
	Napi::Function callback;
	Napi::Function complete;
};

struct RunPreparedTask : public Task {
	RunPreparedTask(Statement &statement_, guinsoodb::unique_ptr<StatementParam> params_, RunType run_type_)
	...
	void Callback() override {
		...
		case RunType::ALL: {
			auto materialized_result = (guinsoodb::MaterializedQueryResult *)result.get();
			Napi::Array result_arr(Napi::Array::New(env, materialized_result->collection.Count()));

			guinsoodb::idx_t out_idx = 0;
			while (true) {
				auto chunk = result->Fetch();
				if (!chunk || chunk->size() == 0) {
					break;
	...
	std::unique_ptr<guinsoodb::QueryResult> result;
	guinsoodb::unique_ptr<StatementParam> params;
	RunType run_type;
};

guinsoodb::unique_ptr<StatementParam> Statement::HandleArgs(const Napi::CallbackInfo &info) {
	size_t start_idx = ignore_first_param ? 1 : 0;
	auto params = guinsoodb::make_unique<StatementParam>();
...
Napi::Value Statement::All(const Napi::CallbackInfo &info) {
	connection_ref->database_ref->Schedule(info.Env(),
	                                       guinsoodb::make_unique<RunPreparedTask>(*this, HandleArgs(info), RunType::ALL));
	return info.This();
}
...
Napi::Value Statement::Run(const Napi::CallbackInfo &info) {
	...
	connection_ref->database_ref->Schedule(info.Env(),
	                                       guinsoodb::make_unique<RunPreparedTask>(*this, HandleArgs(info), RunType::RUN));
	return info.This();
}
...
Napi::Value Statement::Each(const Napi::CallbackInfo &info) {
	...
	connection_ref->database_ref->Schedule(
	    info.Env(), guinsoodb::make_unique<RunPreparedTask>(*this, HandleArgs(info), RunType::EACH));
	return info.This();
}
...
Napi::Value Statement::Finalize_(const Napi::CallbackInfo &info) {
	...
	connection_ref->database_ref->Schedule(env, guinsoodb::make_unique<FinalizeTask>(*this, callback));
	return env.Null();
}
