...
struct OpenTask : public Task {
	...
	void DoWork() override {
		try {
			Get<Database>().database = guinsoodb::make_unique<guinsoodb::GuinsooDB>(filename);
			success = true;
...
Database::Database(const Napi::CallbackInfo &info) : Napi::ObjectWrap<Database>(info), task_inflight(false) {
	...
	Schedule(env, guinsoodb::make_unique<OpenTask>(*this, filename, callback));
}
...
void Database::Process(Napi::Env env) {
	...
	napi_create_async_work(env, NULL, Napi::String::New(env, "guinsoodb.Database.Task"), task_execute, task_complete,
...
Napi::Value Database::Wait(const Napi::CallbackInfo &info) {
	Schedule(info.Env(), guinsoodb::make_unique<WaitTask>(*this, info[0].As<Napi::Function>()));
	return info.This();
}
...
Napi::Value Database::Close(const Napi::CallbackInfo &info) {
	...
	Schedule(info.Env(), guinsoodb::make_unique<CloseTask>(*this, callback));
