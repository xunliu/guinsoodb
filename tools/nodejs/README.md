# GuinsooDB Node Bindings

This package provides a node.js API for [GuinsooDB](https://github.com/cwida/guinsoodb), the "SQLite for Analytics". The API for this client is somewhat compliant to the SQLite node.js client for easier transition (and transition you must eventually).

Load the package and create a database object:

```js
var guinsoodb = require('guinsoodb');

var db = new guinsoodb.Database(':memory:'); // or a file name for a persistent DB
```
