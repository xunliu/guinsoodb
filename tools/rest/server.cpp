...
void print_help() {
	fprintf(stderr, "🦆 Usage: guinsoodb_rest_server\n");
...
struct RestClientState {
	unique_ptr<guinsoodb::QueryResult> res;
	unique_ptr<guinsoodb::Connection> con;
	time_t touched;
};
...
void serialize_json(const Request &req, Response &resp, json &j) {
	...
	j["guinsoodb_version"] = GuinsooDB::SourceID();
...
void sleep_thread(guinsoodb::Connection *conn, bool *is_active, int timeout_duration) {
...
int main(int argc, char **argv) {
	...
	GuinsooDB guinsoodb(dbfile.empty() ? nullptr : dbfile.c_str(), &config);
	ExtensionHelper::LoadAllExtensions(guinsoodb);
	...
	svr.Get("/query", [&](const Request &req, Response &resp) {
		...
		state.con = make_unique<guinsoodb::Connection>(guinsoodb);
	...
	svr.Get("/close", [&](const Request &req, Response &resp) {
		auto ref = req.get_param_value("ref");
		guinsoodb::Connection conn(guinsoodb);
		json j;
		std::lock_guard<std::mutex> guard(client_state_map_mutex);
