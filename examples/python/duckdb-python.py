...
# make this data frame available as a view in guinsoodb
...
rel = guinsoodb.from_csv_auto(temp_file_name)
print(rel)
...
print(guinsoodb.filter(test_df, 'i > 1'))
print(guinsoodb.project(test_df, 'i +1'))
print(guinsoodb.order(test_df, 'j'))
print(guinsoodb.limit(test_df, 2))

print(guinsoodb.aggregate(test_df, "sum(i)"))
print(guinsoodb.distinct(test_df))

# when chaining only the first call needs to include the data frame parameter
print(guinsoodb.filter(test_df, 'i > 1').project('i + 1').order('j').limit(2))
...
# create a table in guinsoodb from the relation
...
res = guinsoodb.query(test_df, 'my_name_for_test_df', 'SELECT * FROM my_name_for_test_df')
print(res.df())
