...
guinsoodb_connection::guinsoodb_connection(string &conninfo) {
	// in-memory database
	database = make_unique<GuinsooDB>(nullptr);
	database->LoadExtension<TPCHExtension>();
	connection = make_unique<Connection>(*database);
}

void guinsoodb_connection::q(const char *query) {
...
schema_guinsoodb::schema_guinsoodb(std::string &conninfo, bool no_catalog) : guinsoodb_connection(conninfo) {
...
dut_guinsoodb::dut_guinsoodb(std::string &conninfo) : guinsoodb_connection(conninfo) {
...
void dut_guinsoodb::test(const std::string &stmt) {
	is_active = true;
