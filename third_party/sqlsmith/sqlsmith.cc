...
int main(int argc, char *argv[]) {
	...
	regex optregex("--(help|verbose|target|guinsoodb|monetdb|version|dump-"
	...
	if (options.count("help")) {
		cerr << "    --guinsoodb=URI         SQLite database to send queries to" << endl
	...
	try {
		...
		if (options.count("guinsoodb")) {
			schema = make_shared<schema_guinsoodb>(options["guinsoodb"], options.count("exclude-catalog"));
		} else {
			cerr << "No GuinsooDB database specified!" << endl;
			return 1;
		}
		...
		if (options.count("guinsoodb")) {
			dut = make_shared<dut_guinsoodb>(options["guinsoodb"]);
		} else {
			cerr << "No GuinsooDB database specified!" << endl;
			return 1;
		}
