...
struct MiniZStream {
	MiniZStream() : type(MiniZStreamType::MINIZ_TYPE_NONE) {
		memset(&stream, 0, sizeof(guinsoodb_miniz::mz_stream));
	}
	~MiniZStream() {
		switch(type) {
		case MiniZStreamType::MINIZ_TYPE_INFLATE:
			guinsoodb_miniz::mz_inflateEnd(&stream);
			break;
		case MiniZStreamType::MINIZ_TYPE_DEFLATE:
			guinsoodb_miniz::mz_deflateEnd(&stream);
			break;
		default:
			break;
		}
	...
	void FormatException(const char *error_msg, int mz_ret) {
		auto err = guinsoodb_miniz::mz_error(mz_ret);
	...
	void Decompress(const char *compressed_data, size_t compressed_size, char *out_data, size_t out_size) {
		...
		if (mz_ret != guinsoodb_miniz::MZ_OK) {
		...
		mz_ret = mz_inflate(&stream, guinsoodb_miniz::MZ_FINISH);
		if (mz_ret != guinsoodb_miniz::MZ_OK && mz_ret != guinsoodb_miniz::MZ_STREAM_END) {
			FormatException("Failed to decompress GZIP block", mz_ret);
		}
	}
	size_t MaxCompressedLength(size_t input_size) {
		return guinsoodb_miniz::mz_compressBound(input_size) + GZIP_HEADER_MINSIZE + GZIP_FOOTER_SIZE;
	}
	void Compress(const char *uncompressed_data, size_t uncompressed_size, char *out_data, size_t *out_size) {
		auto mz_ret = mz_deflateInit2(&stream, guinsoodb_miniz::MZ_DEFAULT_LEVEL, MZ_DEFLATED, -MZ_DEFAULT_WINDOW_BITS, 1, 0);
		if (mz_ret != guinsoodb_miniz::MZ_OK) {
			FormatException("Failed to initialize miniz", mz_ret);
		}
		...
		mz_ret = mz_deflate(&stream, guinsoodb_miniz::MZ_FINISH);
		if (mz_ret != guinsoodb_miniz::MZ_OK && mz_ret != guinsoodb_miniz::MZ_STREAM_END) {
			FormatException("Failed to compress GZIP block", mz_ret);
		}
		auto gzip_footer = gzip_body + stream.total_out;
		auto crc = guinsoodb_miniz::mz_crc32(MZ_CRC32_INIT, (const unsigned char*) uncompressed_data, uncompressed_size);
...
private:
	...
	guinsoodb_miniz::mz_stream stream;
