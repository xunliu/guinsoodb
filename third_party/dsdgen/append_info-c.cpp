...
static void append_value(append_info *info, guinsoodb::Value v) {
	auto append_info = (tpcds_append_information *)info;
	append_info->appender.Append<guinsoodb::Value>(v);
}
...
void append_date(append_info *info, int64_t value) {
	...
	auto ddate = guinsoodb::Date::FromDate(dTemp.year, dTemp.month, dTemp.day);
