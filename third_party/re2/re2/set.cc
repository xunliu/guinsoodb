...
int RE2::Set::Add(const StringPiece& pattern, std::string* error) {
  ...
  Regexp::ParseFlags pf = static_cast<Regexp::ParseFlags>(
    options_.ParseFlags());
  RegexpStatus status;
  guinsoodb_re2::Regexp* re = Regexp::Parse(pattern, pf, &status);
  if (re == NULL) {
    if (error != NULL)
      *error = status.Text();
  ...
  guinsoodb_re2::Regexp* m = guinsoodb_re2::Regexp::HaveMatch(n, pf);
  if (re->op() == kRegexpConcat) {
    int nsub = re->nsub();
    PODArray<guinsoodb_re2::Regexp*> sub(nsub + 1);
    for (int i = 0; i < nsub; i++)
      sub[i] = re->sub()[i]->Incref();
    sub[nsub] = m;
    re->Decref();
    re = guinsoodb_re2::Regexp::Concat(sub.data(), nsub + 1, pf);
  } else {
    guinsoodb_re2::Regexp* sub[2];
    sub[0] = re;
    sub[1] = m;
    re = guinsoodb_re2::Regexp::Concat(sub, 2, pf);
  }
...
bool RE2::Set::Compile() {
  ...
  PODArray<guinsoodb_re2::Regexp*> sub(size_);
  ...
  Regexp::ParseFlags pf = static_cast<Regexp::ParseFlags>(
    options_.ParseFlags());
  guinsoodb_re2::Regexp* re = guinsoodb_re2::Regexp::Alternate(sub.data(), size_, pf);
